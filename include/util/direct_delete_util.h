...
#ifndef GRAPH_SEARCH_DIRECT_DELETE_UTIL_H
#define GRAPH_SEARCH_DIRECT_DELETE_UTIL_H
...
//    static void query_hnsw(hnswlib::HierarchicalNSW<float>& index, const std::vector<std::vector<float>>& queries, int dim, int k, int num_threads, std::vector<std::vector<size_t>>& labels, std::vector<double>& query_times);
    ...
    static void directDeleteMultiThread(hnswlib::HierarchicalNSW<float>& index, const std::vector<size_t>& delete_indices, const std::unordered_map<size_t, size_t>& index_map, int num_threads);
...
#endif //GRAPH_SEARCH_DIRECT_DELETE_UTIL_H
