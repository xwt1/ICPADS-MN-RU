...
//
//
//        std::vector<tableint> getConnectionsWithoutLock(tableint internalId, int level) {
...
//
//
//        std::list<tableint> filterReverseLinklistAndUpdateByDis(tableint internalLabel, int layer, size_t candidates_size) {
//            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> candidateSet;
//            auto reverseLinklist = reverse_linklist_[internalLabel][layer];
//
//
//            for (auto it = reverseLinklist.begin(); it != reverseLinklist.end(); ++it) {
//                tableint candidate_id = *it;
//                dist_t dist = fstdistfunc_(getDataByInternalId(internalLabel), getDataByInternalId(candidate_id), dist_func_param_);
//                // 这个地方考虑一下是dist还是-dist
//                candidateSet.emplace(dist, candidate_id);
//                if(candidateSet.size() > candidates_size){
//                    candidateSet.pop();
//                }
//            }
//
//            // Filter top M0 or M neighbors
////            getNeighborsByHeuristic2(candidateSet, layer == 0 ? maxM0_ : maxM_);
//
//            std::list<tableint> newReverseLinklist;
//            std::unordered_set<tableint> retainedNeighbors;
//            while (!candidateSet.empty()) {
//                retainedNeighbors.insert(candidateSet.top().second);
//                newReverseLinklist.push_back(candidateSet.top().second);
//                candidateSet.pop();
//            }
//
//            // Update the adjacency list of nodes not in the retained list
//            for (auto it = reverseLinklist.begin(); it != reverseLinklist.end(); ++it) {
//                if (retainedNeighbors.find(*it) == retainedNeighbors.end()) {
////                    std::unique_lock<std::mutex> lock(link_list_locks_[*it]);
//                    linklistsizeint* ll_other = get_linklist_at_level(*it, layer);
//                    int size = getListCount(ll_other);
//                    tableint* data = (tableint*)(ll_other + 1);
//
//                    // Create a new list without the internalLabel
//                    std::vector<tableint> newLinks;
//                    newLinks.reserve(size);
//                    for (int i = 0; i < size; ++i) {
//                        if (data[i] != internalLabel) {
//                            newLinks.push_back(data[i]);
//                        }
//                    }
//
//                    // Update the link list
//                    setListCount(ll_other, newLinks.size());
...
//            }
//
//            return newReverseLinklist;
//        }
//
//
//        /*
//         * Directly delete node by external label (single thread) 最新
//         */
...
//                size_t candidate_size = layer == 0 ? maxM0_ : maxM_;
//
//                if(reverseLinklist.size() > candidate_size){
//                    reverseLinklist = filterReverseLinklistAndUpdateByDis(internalLabel,layer, candidate_size);
//                }
//
...
//
//                    getNeighborsByHeuristic2_direct_delete(candidates, candidate_size ,1.1);
...
//                        }
//                    }
//                    reverse_linklist_[internalLabel][layer].clear();
//                }
//            }
//        }
//
...
//
//
//        void exportGraphNodes(const std::string &filename) const {
//            std::ofstream output(filename);
//            if (!output.is_open()) {
//                throw std::runtime_error("Cannot open file to export graph nodes");
//            }
//
//            for (int level = 0; level <= maxlevel_; ++level) {
//                output << "Level " << level << "\n";
...
//                    if (candidate_id == 0) continue;
        ...
        */
        ...
        void updatePoint(const void *dataPoint, tableint internalId, float updateNeighborProbability) {
            ...
            for (int layer = 0; layer <= elemLevel; layer++) {
                int candidate_size = layer == 0 ? maxM0_ : maxM_;
...
//                    }
                ...
                std::unordered_set<tableint> newsNeigh;
                for(auto &&neigh: sNeigh){
                    std::vector<tableint> sNeighVec = getConnectionsWithLock(neigh, layer);
                    bool check_has_internal = false;
                    for(auto &sNeigh :sNeighVec){
                        if(sNeigh == internalId){
                            check_has_internal = true;
                            break;
                        }
                    }
                    if(check_has_internal){
                        newsNeigh.insert(neigh);
                    }
                }
                sNeigh = newsNeigh;


                std::unordered_set<tableint> tempCand = sCand;
                for (auto&& neigh : sNeigh) {
                    // if (neigh == internalId)
                    //     continue;
                    sCand = tempCand;
                    std::vector<tableint> prime_neigh_vec = getConnectionsWithLock(neigh, layer);
                    for (auto& prime_neigh : prime_neigh_vec) {
                        sCand.insert(prime_neigh);
                    }
