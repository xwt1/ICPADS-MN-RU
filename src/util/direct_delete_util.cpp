//
// Created by root on 6/16/24.
//

#include "direct_delete_util.h"


std::vector<std::vector<float>> directDeleteUtil::load_fvecs(const std::string &filename, int &dim, int &num) {
...
std::vector<std::vector<size_t>> directDeleteUtil::load_ivecs_indices(const std::string& filename) {
    std::ifstream input(filename, std::ios::binary);
    if (!input) {
...
void directDeleteUtil::query_hnsw(hnswlib::HierarchicalNSW<float>& alg_hnsw, const std::vector<std::vector<float>>& queries, int k, int num_threads, std::vector<std::vector<size_t>>& results) {
...
void directDeleteUtil::query_hnsw_single(hnswlib::HierarchicalNSW<float>& index, const std::vector<std::vector<float>>& queries, int dim, int k, std::vector<std::vector<size_t>>& labels, std::vector<double>& query_times) {
    size_t num_queries = queries.size();
...
std::vector<std::pair<std::vector<size_t>, std::vector<float>>> directDeleteUtil::query_index(hnswlib::HierarchicalNSW<float>* index, const std::vector<std::vector<float>> &queries, int k) {
...
void directDeleteUtil::directDeleteMultiThread(hnswlib::HierarchicalNSW<float>& index, const std::vector<size_t>& delete_indices, const std::unordered_map<size_t, size_t>& index_map, int num_threads) {
...
void directDeleteUtil::addPointsMultiThread(hnswlib::HierarchicalNSW<float>& index, const std::vector<std::vector<float>>& points, const std::vector<size_t>& labels, int num_threads) {
...
void directDeleteUtil::writeCSVOut(const std::string& filename, const std::vector<std::vector<std::string>>& data) {
...
void directDeleteUtil::writeCSVApp(const std::string& filename, const std::vector<std::vector<std::string>>& data) {
    std::filesystem::path file_path(filename);
    std::filesystem::path dir_path = file_path.parent_path();
...
float directDeleteUtil::recall_score(const std::vector<std::vector<size_t>>& ground_truth, const std::vector<std::vector<size_t>>& predictions, const std::unordered_map<size_t, size_t>& index_map, size_t data_size) {
    size_t hit_count = 0;
...
void directDeleteUtil::knn_thread(const std::vector<std::vector<float>>& data, const std::vector<std::vector<float>>& queries, int dim, int k, size_t start_idx, size_t end_idx, std::vector<std::vector<size_t>>& indices) {
...
std::vector<std::vector<size_t>> directDeleteUtil::brute_force_knn(const std::vector<std::vector<float>>& data, const std::vector<std::vector<float>>& queries, int dim, int k) {
    size_t num_data = data.size();
    size_t num_queries = queries.size();
...
void directDeleteUtil::save_knn_to_ivecs(const std::string& filename, const std::vector<std::vector<size_t>>& knn_results) {
...
void directDeleteUtil::create_directories(const std::vector<std::string>& paths) {
...
std::vector<std::vector<size_t>> directDeleteUtil::generate_unique_random_numbers(int limit, int size, int num) {
    if (size > limit + 1) {
    ...
    for (int n = 0; n < num; ++n) {
        std::vector<size_t> available_numbers(limit + 1);
        std::iota(available_numbers.begin(), available_numbers.end(), 0); // Fill with 0, 1, ..., limit
...
void directDeleteUtil::save_to_fvecs(const std::string& filename, const std::vector<std::vector<int>>& data) {
...
void directDeleteUtil::save_to_ivecs(const std::string& filename, const std::vector<std::vector<size_t>>& data) {
    std::ofstream outfile(filename, std::ios::binary);
    if (!outfile.is_open()) {
        throw std::runtime_error("Could not open file for writing");
    }
    ...
    for (const auto& numbers : data) {
        ...
        outfile.write(reinterpret_cast<const char*>(&size), sizeof(int)); // 写入大小
...
std::vector<std::vector<size_t>> directDeleteUtil::load_ivecs(const std::string &filename, int &dim, int &num) {
    std::ifstream input(filename, std::ios::binary);
    if (!input) {
        std::cerr << "无法打开文件: " << filename << std::endl;
        return {};
    ...
    input.read(reinterpret_cast<char*>(&dim), sizeof(int)); // 读取第一个维度信息
    input.seekg(0, std::ios::end);
    size_t file_size = input.tellg();
    input.seekg(0, std::ios::beg);
    ...
    while (input.peek() != EOF) {
        int temp_dim;
        input.read(reinterpret_cast<char*>(&temp_dim), sizeof(int)); // 读取每个向量的维度信息
        input.seekg(temp_dim * sizeof(size_t), std::ios::cur); // 跳过当前向量的数据
        num++;
    }
    ...
    input.clear();
    input.seekg(0, std::ios::beg);
    input.read(reinterpret_cast<char*>(&dim), sizeof(int)); // 重新读取第一个维度信息

    std::vector<std::vector<size_t>> data(num, std::vector<size_t>(dim));
    for (int i = 0; i < num; ++i) {
        int current_dim;
