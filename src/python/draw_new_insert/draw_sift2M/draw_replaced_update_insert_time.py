...
def plot_replaced_update_time(csv_files, colors, markers, labels, output_path):
    ...
    for csv_file, color, marker, label in zip(csv_files, colors, markers, labels):
        ...
        ax.plot(df['iteration_number'], df['avg_sum_delete_add_time'], linestyle='-', color=color, marker=marker, label=label)
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(args.root_path, 'output', 'new_insert', 'sift_2M', 'replaced_update_time_plot.png')
