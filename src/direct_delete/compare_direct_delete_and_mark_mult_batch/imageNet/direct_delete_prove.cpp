...
//    std::string index_path = root_path + "/data/imageNet/hnsw_maintenance/imageNet_hnsw_maintenance_index.bin";
    std::string index_path = root_path + "/data/imageNet/hnsw_prime/imageNet_hnsw_prime_index.bin";
    std::string ground_truth_path = root_path + "/data/imageNet/imageNet_groundtruth.ivecs";
    std::string output_csv_path = root_path + "/output/figure_3/imageNet/method9_direct_delete.csv";
    std::string output_index_path = root_path + "/output/figure_3/imageNet/method9_imageNet_after_mult_index.bin";

    std::vector<std::string> paths_to_create ={output_csv_path,output_index_path};
    directDeleteUtil::create_directories(paths_to_create);
    ...
    for (int iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        directDeleteUtil::addPointsMultiThread(index, deleted_vectors, new_indices, num_threads);
        ...
        auto results = directDeleteUtil::query_index(&index, queries_tmp, data.size());
