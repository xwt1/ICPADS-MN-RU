...
//    std::string ground_truth_path = root_path + "/data/gist/gist_groundtruth.ivecs";
    std::string output_csv_path = root_path + "/output/back_up_index/gist/edge_connected_replaced_update9.csv";
//    std::string output_index_path = root_path + "/output/full_coverage/gist/edge_connected_replaced_update9_gist_full_coverage_index.bin";
...
//    util::create_directories(paths_to_create);
    ...
    hnswlib::HierarchicalNSW<float> main_index(&space, index_path, false, data_siz, true);
    std::unique_ptr<hnswlib::HierarchicalNSW<float>> backup_index = std::make_unique<hnswlib::HierarchicalNSW<float>>(&space, 1, main_index.M_, main_index.ef_construction_);
//    hnswlib::HierarchicalNSW<float> back_index();
...
//    std::vector<std::vector<size_t>> ground_truth = util::load_ivecs_indices(ground_truth_path);
    ...
    for (int iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        util::markDeleteMultiThread(main_index, delete_indices, index_map, num_threads);
        ...
        util::addPointsMultiThread(main_index, deleted_vectors, new_indices, num_threads);
...
//        std::cout << "Avg Query Time: " << avg_query_time << " seconds\n";
        ...
        auto results = util::query_index(&main_index, queries_tmp, data.size());
        ...
        // rebuild back_up index
        if(!(iteration % 4)){
            std::vector<size_t> labels = results[0].first;
            for(size_t i = 0; i < labels.size() ; i++){
                if(labels[i] >= data_siz){
                    labels[i] -= data_siz;
                }
            }
            std::vector<size_t> back_up_labels;
            std::unordered_set <size_t> labels_set(labels.begin(),labels.end());
            for(size_t i = 0 ;i < data_siz ; i++){
                if(labels_set.find(i) == labels_set.end()){
                    back_up_labels.push_back(i);
                }
            }
            util::backup_index_build(space,backup_index,data,back_up_labels);
        }


        // dual search validation
        size_t found_points = util::dual_search_validation(main_index,backup_index,data_siz,queries_tmp);


        std::string iteration_string = std::to_string(iteration + 1);
        std::string unreachable_points_string = std::to_string(data_siz - found_points);
...
//        std::string avg_query_time_string = std::to_string(avg_query_time);
        ...
        std::vector<std::vector <std::string>> result_data = {{iteration_string, unreachable_points_string}};
